...
class DialogManager {
   ...
   */
  async selectUser() {
    return this.#messageManager.send(commands.MessageCommands.selectUser, {
      mult: false
    });
  }
  /**
  ...
  async selectAccess(blockedAccessPermissions = []) {
    return this.#messageManager.send(commands.MessageCommands.selectAccess, {
  ...
  async selectCRM(params) {
    const response = await this.#messageManager.send(commands.MessageCommands.selectCRM, {
