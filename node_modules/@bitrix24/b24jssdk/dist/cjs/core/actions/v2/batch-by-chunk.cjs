...
class BatchByChunkV2 extends abstractBatch.AbstractBatch {
  static {
  ...
  async make(options) {
    ...
    const opts = {
      ...
      apiVersion: b24.ApiVersion.v2
    };
    const result$1 = new result.Result();
    const dataResult = [];
    const chunks = this.chunkArray(options.calls, batchSize);
    for (const chunkRequest of chunks) {
      const response = await this._b24.getHttpClient(b24.ApiVersion.v2).batch(chunkRequest, opts);
      if (!response.isSuccess) {
        this._addBatchErrorsIfAny(response, result$1);
      }
    ...
    return result$1.setData(dataResult);
  }
}

exports.BatchByChunkV2 = BatchByChunkV2;
//# sourceMappingURL=batch-by-chunk.cjs.map
