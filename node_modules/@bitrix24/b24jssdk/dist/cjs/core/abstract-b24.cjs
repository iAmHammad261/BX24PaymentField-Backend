...
const manager = require('./actions/manager.cjs');
const manager$1 = require('./tools/manager.cjs');
...
class AbstractB24 {
  ...
  constructor() {
    ...
    this._actionsManager = new manager.ActionsManager(this);
    this._toolsManager = new manager$1.ToolsManager(this);
  }
  /**
   * @inheritDoc
   */
  ...
  async callListMethod(method, params, progress, customKeyForResult) {
    ...
    if (type.Type.isFunction(progress) && null !== progress) {
      progress(0);
    }
    const sendParams = {
      ...params,
    ...
    return this.actions.v2.call.make({
      ...
      if (response.isMore()) {
        ...
        while (true) {
          responseLoop = await responseLoop.getNext(this.getHttpClient(b24.ApiVersion.v2));
  ...
  async setRestrictionManagerParams(params) {
    const promises = versionManager.versionManager.getAllApiVersions().map(
...
//# sourceMappingURL=abstract-b24.cjs.map
