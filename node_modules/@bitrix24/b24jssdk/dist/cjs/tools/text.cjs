...
const uuidv7 = require('./uuidv7.cjs');
...
class TextManager {
  ...
  toNumber(value) {
    ...
    if (type.Type.isNumber(parsedValue)) {
      return parsedValue;
    }
    return 0;
  }
  ...
  toBoolean(value, trueValues = []) {
    const transformedValue = type.Type.isString(value) ? value.toLowerCase() : value;
    return ["true", "y", "1", 1, true, ...trueValues].includes(transformedValue);
  }
  toCamelCase(str) {
    if (!type.Type.isStringFilled(str)) {
      return str;
    }
    const regex = /[-_\s]+(.)?/g;
    if (!regex.test(str)) {
  ...
  toDateTime(dateString, template, opts) {
    if (!(typeof template === "undefined") && type.Type.isStringFilled(template)) {
  ...
  toB24Format(date) {
    if (typeof date === "string") {
      ...
      return this.toB24Format(luxon.DateTime.fromJSDate(date));
    }
